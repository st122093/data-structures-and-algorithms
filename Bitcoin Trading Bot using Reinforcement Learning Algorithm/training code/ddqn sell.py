...
class DQN():
    def __init__(self):
        ...
        self.model_target_sell = copy.deepcopy(self.model_sell)
        self.optimizer_sell = torch.optim.Adam(self.model_sell.parameters(), LR)

    def copy_target_sell(self):
        self.model_target_sell.load_state_dict(self.model_sell.state_dict())
    ...
    def target_predict_sell(self, s):
        s = torch.FloatTensor(s).cuda()
        self.model_target_sell.eval()
        with torch.no_grad():
            return self.model_target_sell(s)
        ...
        if len(memory.memory_sell) >= REPLAY_SIZE:
            ...
            for data in replay_data:
                ...
                if data['end']:
                    ...
                    q_values_next = self.target_predict_sell(data['next_state'])
    ...
    while True:
        ...
        while True:
            ...
            if step % TARGET_UPDATE_INTERVAL == 0:
                estimator.copy_target_sell()
                curr_timestamp = time.time()
                curr_time = time.ctime(curr_timestamp)
                print(f'[{curr_time}] STEP: {step} [TARGET UPDATE]')
                df = df.append({'DATE TIME': curr_time, 'STEP': step, 'TARGET UPDATE': True, 'TIMESTAMP': curr_timestamp}, ignore_index=True)
...
df = pd.DataFrame(columns = ['DATE TIME', 'STEP', 'STEP REWARD SELL', 'LOSS SELL', 'EPSILON',
                             'SUM PLAY SCORE SELL',
                             'BEST SUM PLAY SCORE SELL', 'TARGET UPDATE', 'TIMESTAMP'])
