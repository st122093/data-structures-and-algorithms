...
class DQN():
    def __init__(self):
        ...
        self.model_target_buy = copy.deepcopy(self.model_buy)
        self.optimizer_buy = torch.optim.Adam(self.model_buy.parameters(), LR)

    def copy_target_buy(self):
        self.model_target_buy.load_state_dict(self.model_buy.state_dict())
    ...
    def target_predict_buy(self, s):
        s = torch.FloatTensor(s).cuda()
        self.model_target_buy.eval()
        with torch.no_grad():
            return self.model_target_buy(s)
        ...
        if len(memory.memory_buy) >= REPLAY_SIZE:
            ...
            for data in replay_data:
                ...
                if data['end']:
                    ...
                    q_values_next = self.target_predict_buy(data['next_state'])
...
df = pd.DataFrame(columns = ['DATE TIME', 'STEP', 'STEP REWARD BUY', 'LOSS BUY', 'EPSILON',
                             'SUM PLAY SCORE BUY',
                             'BEST SUM PLAY SCORE BUY', 'TARGET UPDATE', 'TIMESTAMP'])
